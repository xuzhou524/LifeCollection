...
@interface LCRulesLineView () <ValueChangeDelegate>
...
@property (nonatomic, strong) LCRulesLineMovingView *moveView;
@property (nonatomic, strong) LCRulesLineMovingView *moveView2;
...
@implementation LCRulesLineView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        LCRulesLineMovingView *moveView = [[LCRulesLineMovingView alloc] initWithFrame:CGRectMake(0, _yc*10.f/25.4*10, sc_w, SlideHeight)];
        moveView.delegate = self;
        [self addSubview:moveView];
        _moveView = moveView;
        
        
        LCRulesLineMovingView *moveView2 = [[LCRulesLineMovingView alloc] initWithFrame:CGRectMake(0, _yc*10.f/25.4*20, sc_w, SlideHeight)];
        ...
        label1.center = CGPointMake(ScreenWidth/2+50, ScreenHeight/2);
        ...
        label2.center = CGPointMake(ScreenWidth/2-50, ScreenHeight/2);
...
- (void)valueChange
{
    ...
    _ycLabel.text = [NSString stringWithFormat:@"长度: %.2f inch",ycTotal];
    _cmLabel.text = [NSString stringWithFormat:@"长度: %.2f cm",cmTotal];
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i = 0; i < self.CMPointArr.count; i++) {
        LCRulesLineModel *model = self.CMPointArr[i];
    ...
    for (int i = 0; i < self.YCPoineArr.count; i++) {
        LCRulesLineModel *model = self.YCPoineArr[i];
        CGContextMoveToPoint(context, model.fromX, model.fromY);
        CGContextAddLineToPoint(context,model.toX, model.toY);
    }
...
- (NSMutableArray *)YCPoineArr
{
    if (!_YCPoineArr) {
        ...
        for (int i = 0; i < 130; i++) {
            ...
            LCRulesLineModel *model = [[LCRulesLineModel alloc] init];
            model.fromX = 0;
            model.fromY = ScreenHeight - (i * _yc);
            model.toY = ScreenHeight - (i * _yc);
            if (i%5==0) {
